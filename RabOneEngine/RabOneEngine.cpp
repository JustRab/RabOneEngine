...
HRESULT InitDevice()
{
    ...
    hr = CompileShaderFromFile("RabOneEngine.fx", "VS", "vs_4_0", &pVSBlob);
    if (FAILED(hr))
    {
        MessageBox(NULL,
    ...
    hr = CompileShaderFromFile("RabOneEngine.fx", "PS", "ps_4_0", &pPSBlob);
    if (FAILED(hr))
    {
        MessageBox(NULL,
            "El archivo FX no se pudo compilar. Ejecuta el ejecutable desde el directorio que contiene el archivo FX.", "Error", MB_OK);
    ...
    hr = CompileShaderFromFile("RabOneEngine.fx", "ShadowPS", "ps_4_0", &pShadowPSBlob);
    if (FAILED(hr))
    {
        MessageBox(NULL,
...
void CleanupDevice()
{
    ...
    // if (g_DepthStencilView) g_pDepthStencilView->Release();
...
void RenderScene()
{
    ...
    // g_deviceContext.m_deviceContext->ClearRenderTargetView(g_renderTargetView, ClearColor);
    // g_deviceContext.m_deviceContext->ClearDepthStencilView(g_DepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
    // Set Viewport
